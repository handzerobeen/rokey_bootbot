...
class ServerConnectorDesigningNode(Node):
    """server_connector_node.py(실제 production 노드)의 실험용 복제본.

    아래 원본 docstring 그대로 유지:
    ...
    바로 재발행한다. 이 재발행은 _on_stroke_cmd에도 그대로 잡혀서 재실행 자체가
    새 세션으로 이력에 남는다.
    ...
    def __init__(self, node_name='server_connector_node_designing'):
    ...
    def _setup_realtime_feed(self):
        ...
        # 기본값은 calligraphy_robot/calligraphy_config.json의 robot.z_down과 반드시 맞춰야
        # 한다 -- 두 패키지가 서로 의존하지 않는 별도 구현이라 자동으로 동기화되지 않는다.
        # 여기 값이 실제 z_down과 어긋나면 폴링된 z가 이 범위에 절대 안 들어와서 GUI에
        # action이 항상 'move'로만 잡히고 아무것도 그려지지 않는다 (실제로 130.0으로
        # 방치되어 있던 탓에 겪은 문제).
        self.declare_parameter('pen_z_down', 91.0)
        ...
        self._pen_z_down = self.get_parameter('pen_z_down').get_parameter_value().double_value
        self._pen_z_draw_tolerance = self.get_parameter(
            'pen_z_draw_tolerance').get_parameter_value().double_value
    ...
    def _on_current_posx_response(self, future):
        ...
        # 로봇 쪽에서 stroke_id/action을 따로 알려주지 않으므로, z가 pen_z_down에 가까우면
        # 붓이 닿아 있는 상태(draw)로, 아니면 든 상태(move)로 간주해 GUI 캔버스에 선을
        # 그을지 말지를 z값만으로 판단한다.
    ...
    def _on_stroke_cmd(self, msg):
        ...
        points = [{'x': float(x), 'y': float(y)} for x, y in zip(msg.x, msg.y)]
...
def main(args=None):
    ...
    node = ServerConnectorDesigningNode()

    try:
        rclpy.spin(node)
    except (KeyboardInterrupt, ExternalShutdownException):
        node.get_logger().info('server_connector_node_designing 종료')
    finally:
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
